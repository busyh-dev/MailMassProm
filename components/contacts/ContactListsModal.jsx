                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    {incomingContacts.length > 0 && (
                      <button
                        onClick={() => { setIncomingContacts([]); onClearSelection?.(); }}
                        className="text-[11px] text-red-500 hover:text-red-700 font-medium underline transition"
                      >
                        Svuota tutti
                      </button>
                    )}
              ...
              lists.map(list => {
                ...
                return (
                  <div key={list.id} className="border border-gray-200 rounded-xl overflow-hidden">
                    ...
                    {/* Contatti espansi */}
                    {isExpanded && (
                      <div className="border-t border-gray-100 bg-gray-50 p-3 max-h-56 overflow-y-auto">
                        {resolvedContacts.length === 0 ? (
                          <p className="text-xs text-gray-400 text-center py-2">Nessun contatto nella lista</p>
                        ) : (
                          <div>
                            {/* Barra Seleziona Tutti & Azioni in 1 Click */}
                            <div className="flex items-center justify-between pb-2 mb-2 border-b border-gray-200">
                              <label className="flex items-center gap-2 text-xs font-semibold text-gray-700 cursor-pointer select-none">
                                <input
                                  ...
                                  checked={
                                    ...
                                    selectedInList[list.id]?.length === resolvedContacts.length
                                  }
                                  onChange={(e) => {
                                    setSelectedInList(prev => ({
                                      ...prev,
                                      [list.id]: e.target.checked ? resolvedContacts.map(c => c.id) : []
                                    }));
                                  }}
                                  className="rounded border-gray-300 text-red-600 focus:ring-red-500"
                                />
                                <span>Seleziona tutti ({resolvedContacts.length})</span>
                              </label>
                              ...
                              {(selectedInList[list.id]?.length || 0) > 0 && (
                                <button
                                  ...
                                  className="text-xs font-semibold text-white bg-red-600 hover:bg-red-700 px-3 py-1 rounded-lg transition flex items-center gap-1 shadow-sm"
                                >
                                  <Trash2 className="w-3.5 h-3.5" />
                                  Elimina {selectedInList[list.id].length} selezionati in 1 click
                                </button>
                              )}
                            </div>
